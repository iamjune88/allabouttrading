...
| NH가 그날 `01A103`(체결시분) 대신 `02A101`/"정정포함" 같은 변형만 보냄 | 제목 필터(`subject:"국문가정산(체결시분)"`) 쿼리가 0건이면, 필터 없이 재시도해서 받을 수 있는 변형이라도 받는 fallback을 추가했다(시간 컬럼은 비게 됨 → 수기 입력 필요). 그날 체결시분 리포트 자체가 안 온 것뿐이지 파이프라인 버그가 아님 |
| TradingView Desktop의 CDP(9222)가 이전 세션에서 떠 있다가 다음 세션 시작 시 죽어있음 | TV Desktop은 컴퓨터 재부팅/유휴/수동 종료로 꺼지면 CDP도 같이 끊김 — 매 세션 시작 시 `curl localhost:9222/json/version`으로 먼저 확인하고, 죽어있으면 섹션 1의 실행 스크립트로 재실행(재실행하면 심볼/타임프레임은 저장된 레이아웃 기본값으로 돌아가므로 다시 설정해야 함) |
| `tradingview-mcp`의 `tv scroll`(CLI)/`chart_scroll_to_date`가 `evaluate is not defined` 에러 | `src/core/chart.js`의 `scrollToDate`도 `drawing.js`와 같은 버그 클래스 — `_deps`/`_resolve(_deps)` 구조분해가 누락되어 있음. `export async function scrollToDate({ date, _deps })`로 바꾸고 첫 줄에 `const { evaluate } = _resolve(_deps);` 추가해서 패치(역시 원본 저장소 버그, 재클론 시 재발) |
| `tv ohlcv`로 받을 수 있는 5분봉 히스토리가 ~300봉(`total_available`)에서 더 안 늘어남 | `scrollToDate`를 패치해서 과거 날짜로 스크롤해도, 더 기다려도 캡이 풀리지 않음 — TradingView Desktop이 해당 해상도에 대해 메모리에 들고 있는 봉 자체가 그 정도로 제한된 것으로 보임. 실전 영향: 5분봉 기준 SMA20/RSI(14)/세션 VWAP는 문제없이 계산 가능하지만, SMA60은 그날 후반부만, SMA120/SMA200은 시계열 계산 자체가 불가능(TV 현재값 스냅샷을 점선 참조선으로 대신 표시) |
| 자체 계산한 VWAP/RSI 값이 TV 차트 범례에 뜨는 값과 다름 | 2026-06-26 KTB10 기준 VWAP 106.78(자체) vs 107.31(TV), RSI 53.8(자체) vs 64.71(TV) — 세션 앵커 기준이나 RSI 웜업 구간이 TV 내부 로직과 다른 것으로 추정되나 정확한 원인은 미해결. 자체 계산값은 일중 추세/모양 참고용으로만 쓰고, TV 범례값과 정확히 일치한다고 단정하지 말 것 |
...
9. `tradingview-mcp`를 새로 클론했다면 `drawing.js`뿐 아니라 `src/core/chart.js`의 `scrollToDate`도 같은 `_deps` 누락 버그가 있으니 같이 패치한다.
10. 일중 거래 라벨링 차트는 TV 스크린샷 방식과, `tv ohlcv`로 데이터만 받아 matplotlib으로 직접 렌더링하는 방식 **두 가지를 상황에 맞게 병행**한다(하나가 다른 하나를 대체하는 게 아님). 커스텀 렌더링은 지표 범례 가림·우측 여백 불가·라벨 겹침 수동조정 같은 TV UI 한계를 코드로 결정론적으로 해결할 수 있는 대신, SMA120/200처럼 긴 lookback이 필요한 지표는 `tv ohlcv`의 ~300봉 캡 때문에 시계열로는 못 그리고 VWAP/RSI도 TV 범례값과 정확히 일치하지 않을 수 있음(위 "주의사항" 표 참고) — 트레이드오프를 보고 그때그때 선택할 것.
